    ...
    try
        {
            ...
            $stmt = $db->prepare("SELECT DISTINCT EventID FROM EventList WHERE EventID = :eventIDVal;");
            
            $stmt->bindParam(':eventIDVal', $postID, PDO::PARAM_STR);
    ...
    try
        {
            ...
            $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                ...
                $stmt = $db->prepare("UPDATE EventList SET archived = 0 WHERE EventID = :postIDVal;");
...
if($isAdmin)
{
    if(isset($_POST["deleteRadio"]))
    {
        if(doesPostExist($_POST["deleteRadio"]))
        {
            ...
            header("Location: ./unarchiveEvents.php");
        }
        else
        {
