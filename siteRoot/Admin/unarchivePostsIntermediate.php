    ...
    try
        {
            ...
            $stmt = $db->prepare("SELECT DISTINCT id FROM discussions WHERE id = :postIDVal;");
    ...
    try
        {
            ...
            $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                ...
                $stmt = $db->prepare("UPDATE discussions SET archived = 0 WHERE id = :postIDVal;");
                $stmt->bindParam(':postIDVal', $postID, PDO::PARAM_STR);
...
if($isAdmin)
{
    if(isset($_POST["deleteRadio"]))
    {
        if(doesPostExist($_POST["deleteRadio"]))
        {
            ...
            header("Location: ./unarchivePosts.php");
        }
        else
        {
